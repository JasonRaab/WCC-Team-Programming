...
Public Class DAO
    ...
    Public Function GetAllOpenLocalOrders() As List(Of Order)
        'Returns all open local orders that have at least 1 item
        Dim lstAllItems As New List(Of Item)
        Dim lstAllIngredients As New List(Of Ingredient)
        Dim lstOpenLocalOrders As New List(Of Order)
        ...
        'First, get a list of all local orders that are not closed

        connection.Open()
        command.CommandText = "SELECT * FROM dumpster_fire.order_info WHERE order_type = 'dine_in' AND is_open = 1"
        Reader = command.ExecuteReader()
        ...
        While Reader.Read
            ...
            End If
            lstOpenLocalOrders.Add(New Order(order_id, user_id, order_date, order_subtotal, order_tax, order_total, order_type, table_number, is_open))
        End While
        connection.Close()
        ...
        'Remove any orders that have zero items
        'Have to loop backwards to be able to remove things while looping
        For i As Integer = lstOpenLocalOrders.Count - 1 To 0 Step -1
            If lstOpenLocalOrders(i).LstItems.Count <= 0 Then
                lstOpenLocalOrders.RemoveAt(i)
            End If
        Next

        'This block is only debug text
        'Debug.WriteLine("GetAllOpenLocalOrders Debug:")
        'For Each order As Order In lstOpenLocalOrders
        '    Debug.WriteLine("ORDER ID " & order.OrderId & "table number " & order.Location)
        ...
        '    Next
        'Next
        Return lstOpenLocalOrders
    End Function

    Public Function GetAllOpenOnlineOrders() As List(Of Order)
        'Returns all open online orders that have at least 1 item
        Dim lstAllItems As New List(Of Item)
        Dim lstAllIngredients As New List(Of Ingredient)
        ...
        'Remove any orders that have zero items
        'Have to loop backwards to be able to remove things while looping
        For i As Integer = lstOpenOnlineOrders.Count - 1 To 0 Step -1
            If lstOpenOnlineOrders(i).LstItems.Count <= 0 Then
                lstOpenOnlineOrders.RemoveAt(i)
            End If
        Next

        'This block is only debug text
        'Debug.WriteLine("GetAllOpenOnlineOrders Debug:")
        'For Each order As Order In lstOpenOnlineOrders
    ...
    Public Function GetUser(pin As Integer) As User
        Dim user As User
